...
const MyMedicationRequests = ({ onRequestAdded }) => {
  ...
  const handleViewPrescription = async (prescriptionFileUrl) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/medications/prescription/${prescriptionFileUrl}`,
        {
          headers: {
  ...
  const fetchMyRequests = async () => {
    try {
      ...
      if (
        ...
        setError(
          "Access denied. You do not have permission to view medication requests."
        );
      } else {
  ...
  const getStatusBadge = (request) => {
    ...
    if (request.confirmationStatus === "in_progress") {
      return (
    ...
    if (request.confirmationStatus === "done") {
      return (
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600">
            Manage medication requests for your children
          </p>
        </div>
        ...
        {requests.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="text-gray-600 mb-4">
              You haven't submitted any medication requests yet.
            </p>
            <button
              onClick={handleCreateRequest}
              className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors"
                  ...
                  <tr key={request.requestId} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {request.prescriptionFile ? (
                        <button
                          onClick={() =>
                            handleViewPrescription(request.prescriptionFile)
                          }
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex space-x-2">
                        ...
                        {/* Chỉ hiển thị Edit và Delete khi request chưa được confirm/reject và không phải in_progress/done */}
                        {!request.isConfirmed &&
                          !request.confirmed &&
                          !request.isRejected &&
                          !request.rejected &&
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                  ...
                  <div className="grid grid-cols-3 gap-4 mt-3">
                    <div className="text-center">
                      ...
                      <span className="text-lg font-semibold text-blue-600">
                        {selectedRequest.morningQuantity || "N/A"}
                      </span>
                    </div>
                    <div className="text-center">
                      <span className="block text-sm font-medium text-gray-600">
                      ...
                      <span className="text-lg font-semibold text-orange-600">
                        {selectedRequest.noonQuantity || "N/A"}
                      </span>
                    </div>
                    <div className="text-center">
                      <span className="block text-sm font-medium text-gray-600">
                      ...
                      <span className="text-lg font-semibold text-purple-600">
                        {selectedRequest.eveningQuantity || "N/A"}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div>
                ...
                <div className="mt-3 space-y-2">
                  ...
                  {(selectedRequest.isRejected ||
                    ...
                    selectedRequest.confirmationStatus === "rejected") && (
                    <div className="bg-red-50 border border-red-200 rounded-md p-3 mt-2">
                      <p className="text-red-800 text-sm font-medium">
                        This medication request has been rejected by the school
                        nurse.
                      </p>
                      {(selectedRequest.rejectionReason ||
                        ...
                        <p className="text-red-700 text-sm mt-1">
                          You may submit a new request after addressing the
                          concerns mentioned in the rejection reason.
                        </p>
                      )}
                    </div>
                  )}
            ...
            <div className="mt-6 flex justify-end space-x-3">
              ...
              {/* Chỉ hiển thị Edit button khi request chưa được confirm/reject và không phải in_progress/done */}
              {!selectedRequest.isConfirmed &&
                ...
                selectedRequest.confirmationStatus !== "rejected" &&
                selectedRequest.confirmationStatus !== "in_progress" &&
                selectedRequest.confirmationStatus !== "done" && (
                  <button
                    onClick={() => {
