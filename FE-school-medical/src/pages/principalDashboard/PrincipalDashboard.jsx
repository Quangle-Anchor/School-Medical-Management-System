...
const PrincipalDashboard = () => {
  ...
  const getCurrentViewFromURL = () => {
    ...
    if (path.includes("/notifications")) return "notifications"; // Added notifications view
  ...
  const [isLoadingHealthIncidents, setIsLoadingHealthIncidents] =
    useState(true);
  ...
  useEffect(() => {
    ...
    const fetchStaffData = async () => {
      try {
        ...
        const principalStaff = users.filter(
          (user) => user.roleName === "Principal"
        ).length;
        const parentStaff = users.filter(
          (user) => user.roleName === "Parent"
        ).length;
    ...
    const fetchLowStockItems = async () => {
      try {
        ...
        const lowStockItems = inventory.filter((item) => {
          ...
          return (
            (item.status || inventoryAPI.getStockStatus(actualQuantity)) ===
            "low"
          );
        });
  ...
  const principalCardData = [
...

    {
      ...
      change: isLoadingHealthIncidents
        ? "Loading..."
  ...
  // Memoize principal user object to avoid unnecessary re-renders
  const principalId = localStorage.getItem("userId");
  const principalRole = localStorage.getItem("role");
  const principalName = localStorage.getItem("fullname");
  const principalUser = useMemo(
    ...
    [principalId, principalRole, principalName]
  );
  ...
  const renderContent = () => {
    switch (activeView) {
      ...
      case "notifications":
        return <NotificationsView user={principalUser} />;
